...
@section('title', 'tagihan Penyedotan')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header"> <div class="d-flex justify-content-end">
                    <a href="{{ route('cetak.tagihanpenyedotan') }}" target="_blank" class="btn btn-success btn-md mr-2">
                        <i class="fas fa-file-pdf"></i> View Laporan
                    </a>
                    {{-- <a  class="btn btn-success btn-md mr-2">
                        <i class="fas fa-file-excel"></i> Export Excel
                        href="{{ route('tagihanPenyedotan.exportExcel') }}"
                    </a> --}}
                    @include('tagihanpenyedotan.create')
                </div>
                </div>
                <div class="card-body">
    ...
    $(document).ready(function() {
        var table = $('.table').DataTable({
            ...
            // paging: false,        // Matikan pagination (opsional)
                ajax: {
                    url: "{{ route('get.tagihanpenyedotan') }}",
                    type: 'GET'
                },
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
                    {data: 'proses_petugas_id', name: 'proses_petugas_id'},
                    {data: 'nama_bangunan', name: 'bangunan.nama'},
